...
__device__ bool Object::rayTrace(Ray ray, float3 &hit_point, float3 &normal) {
	printf("error: raytracing object\n");
	return false;
}
__device__ float3 Object::get_color(float3 frag_point)
{
	return color;
}
...
__device__ Triangle::Triangle(float3 vtx0, float3 vtx1, float3 vtx2, float2 anchor0, float2 anchor1, float2 anchor2)
	: v0{ vtx0 }, v1{ vtx1 }, v2{ vtx2 }, a0{ anchor0 }, a1{ anchor1 }, a2{ anchor2 } {}
...
__device__ float3 Triangle::get_color(float3 frag_point) {
	//float3 v0vf = frag_point - v0;
	//float3 v0v1 = v1 - v0;
	//float3 v0v2 = v2 - v0;
	//
	//float u = dot(v0vf, v0v1) / length(v0v1);
	//float v = dot(v0vf, v0v2) / length(v0v2);

	//float2 a0a1 = a1 - a0;
	//float2 a0a2 = a2 - a0;
	//float2 pixel_lookup = a0 + (a0a1 * u) + (a0a2 * v);
